...
class SecurityValidator:
    ...
    def validate_shell_syntax(self, command: str) -> Tuple[bool, str]:
        ...
        for pattern in dangerous_commands:
            if re.search(pattern, command, re.IGNORECASE):
                return (
                    False,
                    f"Inherently dangerous command pattern detected: {pattern}",
                )
        ...
        try:
            result = subprocess.run(
                ["bash", "-n"], input=command, text=True, capture_output=True, timeout=5
            )
    ...
    def _is_safe_usage(self, command: str, char: str) -> bool:
        ...
        if char in ["<", ">", "|"] and any(
            safe in command for safe in ["git ", "python ", "npm ", "cat ", "echo "]
        ):
            return True
    ...
    def validate_content_security(
        ...
        if self.zero_width_chars.search(normalized):
            issues.append(
                "Zero-width characters detected (potential invisible injection)"
            )
        ...
        for char in text:
            if char in self.homoglyph_map:
                suspicious_chars.append(
                    f"'{char}' (looks like '{self.homoglyph_map[char]}')"
                )

        if suspicious_chars:
            issues.append(
                f"Potential homoglyph characters: {', '.join(suspicious_chars[:5])}"
            )
            ...
            if ord(char) > 127:  # Non-ASCII character
                try:
                    ...
                    if block_name not in self.safe_unicode_blocks:
                        issues.append(
                            f"Potentially unsafe Unicode character: {char} (block: {block_name})"
                        )
                except ValueError:
...
def safe_execute_command(
    command: str, validator: Optional[SecurityValidator] = None
...
if __name__ == "__main__":
    ...
    for test_input, description in test_cases:
        is_safe, issues = validator.validate_content_security(
            test_input, "shell_command"
        )
